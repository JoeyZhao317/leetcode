 ...
 * We are looking for the longest possible sequence built from elements of A, 
 * A[B[0]], A[B[1]], ..., A[B[K]], satisfying the following conditions: 

 1.The sequence must be decreasing; that is, A[B[0]] > A[B[1]] > ... > A[B[K]]. 
 2.For any two consecutive elements of the sequence, A[B[I]] and A[B[I+1]],
 all the elements of A between them must be smaller than them; that is, 
 for any J = MIN(B[I], B[I+1]) + 1, ..., MAX(B[I], B[I+1]) - 1,
 we have A[J] < A[B[I+1]]. 
 ...
 that, given a zero-indexed array A containing N different integers, 
 computes the maximum length of a sequence satisfying the above conditions. 
